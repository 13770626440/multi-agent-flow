...
### 8.1 OpenClaw 客户端实现方式变更（2026-04-28）

**变更原因**：
OpenClaw Gateway 不提供完整的 REST API（如 `/api/agents`、`/api/cron`、`/api/v1/message` 等端点返回 404），需改用 Docker Exec CLI 方式进行管理操作。

**变更内容**：

| 操作 | 原方案（HTTP API） | 新方案（CLI 命令） |
|:---|:---|:---|
| 列出 Agent | `GET /api/agents` | `openclaw agents list` |
| 创建 Agent | `POST /api/agents` | `openclaw agents add {name} --model {model} --workspace {workspace}` |
| 发送消息 | `POST /api/v1/message` | `openclaw agent --agent {name} -m "{content}"` |
| 配置 Cron | `POST /api/cron` | `openclaw cron add --name {id} --agent {agent} --cron {schedule} --message {msg}` |

**影响范围**：
- `app/clients/openclaw_client.py`：所有写操作方法改为 CLI 执行
- `app/core/agent_provisioner.py`：检查逻辑从 OpenMOSS 改为 OpenClaw

### 8.2 AgentProvisioner 检查逻辑修复（2026-04-28）

**问题描述**：
`check_agent_exists()` 检查的是 **OpenMOSS**（Agent 注册系统），但 `create_agent()` 创建的是 **OpenClaw**（Agent 执行环境）。两个系统独立，检查逻辑错位导致：
- 旧 Agent 一直存在，但入职包没有更新
- 日志显示 "Agent already exists" 但实际是检查错了系统

**修复方案**：
1. 新增 `openclaw_client.list_agents()` 方法，列出 OpenClaw 中已配置的 Agent
2. 修改 `check_agent_exists` 为 `check_agent_exists_in_openclaw`，检查 OpenClaw 而非 OpenMOSS
3. 当 Agent 已存在时，仍然发送入职包更新 AGENTS.md（支持模板更新后重新配置）

**验证结果**：
- ✅ AGENTS.md 成功更新，包含角色名称、默认 prompt、Skills 内容、Workspace 规范
- ✅ Cron 配置成功（每 15 分钟唤醒）
- ✅ 完整流程验证通过：模板加载 → Agent 检查 → 发送入职包 → Cron 配置 → AGENTS.md 更新

### 8.3 OpenMOSS Agent 列表 API 认证修复（2026-04-27）

**问题**：`/api/agents` 返回 422 Unprocessable Entity

**根因**：需要 `Authorization: Bearer <api_key>` header，而不是 `X-Agent-Key`

**修复**：`openmoss_client.list_agents()` 改用正确的认证格式
